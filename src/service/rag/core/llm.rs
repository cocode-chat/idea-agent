...
pub fn get_chat_llm() -> Result<OpenAI<OpenAIConfig>, String> {
    // 从环境变量获取对话模型基础URL
    let conversation_base_url = std::env::var("CHAT_BASE_URL")
        .map_err(|_| "Missing CHAT_BASE_URL environment variable")?;
    
    // 从环境变量获取对话模型名称
    let conversation_model = std::env::var("CHAT_MODEL")
        .map_err(|_| "Missing CHAT_MODEL environment variable")?;
    
    // 从环境变量获取对话模型API密钥
    let conversation_api_key = std::env::var("CHAT_API_KEY")
        .map_err(|_| "Missing CHAT_API_KEY environment variable")?;
